    ...
    switch ($informe->getTipoInforme()->getNombre()) {
      case 'Alumnos':
        if ($informe->getListado() != 1) {
          $this->redirect('informes/busquedaAlumnos?id=' . $informe->getId());
          break;
        } else {
          $this->redirect('informes/busquedaListadoAlumnos?id=' . $informe->getId());
          break;
        }

      case 'Docente':
        $this->redirect('informes/busquedaDocentes?id=' . $informe->getId());
        break;
        break;
      case 'General':
...
//por seguridad: para no mostrar otros datos del usuario como clave, preguntas, etc
  ...
  function isNotAssocArray($arr) {
    return (0 !== array_reduce(
                    array_keys($arr), create_function('$a, $b', 'return ($b === $a ? $a + 1 : 0);'), 0
            )
            );
  }
