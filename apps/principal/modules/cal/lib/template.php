...
class Page {
	...
	function draw_subscribe($template_p) {
		...
		if ($this->cal != $this->ALL_CALENDARS_COMBINED && $this->subscribe_path != '' && $this->download_filename != '') {
			$this->page 	= str_replace('{SUBSCRIBE_PATH}', $this->subscribe_path, $this->page);
			$this->page 	= str_replace('{DOWNLOAD_FILENAME}', $this->download_filename, $this->page);
		} else {
	...
	function draw_print($template_p) {
		...
		$parse_month 	= date ("Ym", strtotime($this->getdate));
		$parse_year 	= date ("Y", strtotime($this->getdate));

		$seen_events = array();
		foreach($this->master_array as $key => $val) {
					...
					if ($new_val2['event_text']) {	
						...
						if (isset($new_val2['display_end'])) $event_end = $new_val2['display_end'];
							if (!$new_val2['event_start']) { 
								$event_start = $this->lang['l_all_day'];
								$event_start2 = '';
								$event_end = '';
							} else {
	...
	function draw_search($template_p) {
		...
		// Pull out each event
		foreach($this->the_arr as $key => $val) {
				...
				if ($description == '') {
					...
					$events_tmp = preg_replace('!<\!-- switch exceptions on -->(.*)<\!-- switch exceptions off -->!is', '', $events_tmp);
						...
						foreach ($val['exceptions'] as $except_val){
							...
							$except_event_end    	= date ($this->timeFormat, strtotime ($except_val['event_end']));
	...
	function draw_week($template_p) {
		...
		// Figure out colspans
		$dayborder 	= 0;
		$thisdate  	= $this->start_week_time;
		$swt	   	= $this->start_week_time;
		for ($i=0;$i<7;$i++) {
		...
		for ($i=0; $i<$this->week_length; $i++) {
			$thisdate 			= date ('Ymd', $this->start_week_time); 
			$weekarray[$i] 		= $thisdate;
			$this->start_week_time 	= strtotime('+1 day', $this->start_week_time);
		}
		...
		foreach ($weekarray as $get_date) {
			...
			if (is_array($this->master_array[$get_date]['-1']) && !empty($this->master_array[$get_date]['-1']) ) {
				foreach ($this->master_array[$get_date]['-1'] as $uid => $allday) {
					$event_calno  	= $allday['calnumber'];
					$event_calno	= (($event_calno - 1) % $this->unique_colors) + 1;
		...
		$start_wt		 	= strtotime(dateOfWeek($this->getdate, $this->week_start_day));
		...
		$event_length = array ();
		$border = 0;
		preg_match ('/([0-9]{4})([0-9]{2})([0-9]{2})/', $this->getdate, $this->day_array2);
		...
		foreach ($this->day_array as $key) {
			...
			$key = mktime($regs_tmp[1],$regs_tmp[2],0,$this->this_month,$this_day,$this->this_year);
			$key = date ($this->timeFormat, $key);
												

			if (ereg("([0-9]{1,2}):00", $key)) {
				$weekdisplay .= '<tr>';
				$weekdisplay .= '<td colspan="4" rowspan="' . (60 / $this->gridLength) . '" align="center" valign="top" width="60" class="timeborder">'.$key.'</td>';
				$weekdisplay .= '<td bgcolor="#a1a5a9" width="1" height="' . $this->gridLength . '"></td>';
			} elseif ($cal_time == $this->day_start) {
				$size_tmp = 60 - (int)substr($cal_time,2,2);
				$weekdisplay .= '<tr>';
				$weekdisplay .= '<td colspan="4" rowspan="' . ($size_tmp / $this->gridLength) . '" align="center" valign="top" width="60" class="timeborder">'.$key.'</td>';
				$weekdisplay .= '<td bgcolor="#a1a5a9" width="1" height="' . $this->gridLength . '"></td>';
			} else {
				$weekdisplay .= '<tr>';
				$weekdisplay .= '<td bgcolor="#a1a5a9" width="1" height="' . $this->gridLength . '"></td>';
			...
			for ($week_loop=0; $week_loop<$this->week_length; $week_loop++) {
				$thisday = date("Ymd", $thisdate);
				$dayborder = 0;
				unset($this_time_arr);

				if ($this->day_start == $cal_time && isset($this->master_array[$thisday]) && is_array($this->master_array[$thisday])) {
					foreach($this->master_array[$thisday] as $time_key => $time_arr) {
					...
					if (isset($this->master_array[$thisday][$cal_time]) && sizeof($this->master_array[$thisday][$cal_time]) > 0) {
						$this_time_arr = $this->master_array[$thisday][$cal_time];
					}
				}
					...
					foreach ($this_time_arr as $eventKey => $loopevent) {
						...
						while (isset($event_length[$thisday][$j])) {
							if ($event_length[$thisday][$j]["state"] == "ended") {
								$event_length[$thisday][$j] = array ("length" => ($drawEvent["draw_length"] / $this->gridLength), "key" => $eventKey, "overlap" => $loopevent["event_overlap"],"state" => "begin");
								break;
							}
							$j++;
						}
						if ($j == sizeof($event_length[$thisday])) {
							array_push ($event_length[$thisday], array ("length" => ($drawEvent["draw_length"] / $this->gridLength), "key" => $eventKey, "overlap" => $loopevent["event_overlap"],"state" => "begin"));
						}
					}
				}
					...
					for ($i=0;$i<sizeof($event_length[$thisday]);$i++) {
						...
						switch ($event_length[$thisday][$i]["state"]) {
							case "begin":
								...
								$event_start 	= date ($this->timeFormat_small, $event_start);
	...
	function draw_day($template_p) {
		...
		if (is_array($this->master_array[$this->getdate]['-1'])) {
			preg_match("!<\!-- loop allday on -->(.*)<\!-- loop allday off -->!is", $this->page, $match1);
			$loop_ad = trim($match1[1]);
			foreach ($this->master_array[$this->getdate]['-1'] as $uid => $allday) {
				$event_calno  	= $allday['calnumber'];
				$event_calno	= (($event_calno - 1) % $this->unique_colors) + 1;
 				$event 			= openevent($this->getdate, -1, $uid, $allday);
				$loop_tmp 		= str_replace('{ALLDAY}', $event, $loop_ad);
				$loop_tmp 		= str_replace('{CALNO}', $event_calno, $loop_tmp);
				$replace		.= $loop_tmp;
			}
		...
		if (isset($this->master_array[($this->getdate)])) {
			foreach($this->master_array[($this->getdate)] as $ovlKey => $ovlValue) {
				if ($ovlKey != '-1') {
					foreach($ovlValue as $ovl2Value) {
		...
		$event_length = array ();
		$border = 0;
		preg_match('/([0-9]{4})([0-9]{2})([0-9]{2})/', $this->getdate, $day_array2);
		$this_day = $day_array2[3]; 
		$this->this_month = $day_array2[2];
		$this->this_year = $day_array2[1];
		foreach ($this->day_array as $key) {
			...
			// add events that overlap $this->day_start instead of cutting them out completely
			if (($this->day_start == $cal_time) && (isset($this->master_array[$this->getdate]))) {
				foreach($this->master_array[$this->getdate] as $time_key => $time_arr) {
					if ((int)$time_key <= (int)$cal_time) {
						if (is_array($time_arr) && $time_key != '-1') {
							foreach($time_arr as $uid => $event_tmp) {
								if ((int)$event_tmp['event_end'] > (int)$cal_time) {
				...
				if (isset($this->master_array[$this->getdate][$cal_time]) && sizeof($this->master_array[$this->getdate][$cal_time]) > 0) {
					$this_time_arr = $this->master_array[$this->getdate][$cal_time];
				}
			}
				...
				foreach ($this_time_arr as $eventKey => $loopevent) {
					...
					while (isset($event_length[$j])) {
						if ($event_length[$j]['state'] == 'ended') {
							$event_length[$j] = array ('length' => ($drawEvent['draw_length'] / $this->gridLength), 'key' => $eventKey, 'overlap' => $loopevent['event_overlap'],'state' => 'begin');
							break;
						}
						$j++;
					}
					if ($j == sizeof($event_length)) {
						array_push ($event_length, array ('length' => ($drawEvent['draw_length'] / $this->gridLength), 'key' => $eventKey, 'overlap' => $loopevent['event_overlap'],'state' => 'begin'));
					}
				}
			}
			if (preg_match('/([0-9]{1,2}):00/', $key)) {
				$daydisplay .= '<tr>'."\n";
				$daydisplay .= '<td rowspan="' . (60 / $this->gridLength) . '" align="center" valign="top" width="60" class="timeborder">'.$key.'</td>'."\n";
				$daydisplay .= '<td bgcolor="#a1a5a9" width="1" height="' . $this->gridLength . '"></td>'."\n";
			} elseif($cal_time == $this->day_start) {
				$size_tmp = 60 - (int)substr($cal_time,2,2);
				$daydisplay .= '<tr>'."\n";
				$daydisplay .= "<td rowspan=\"" . ($size_tmp / $this->gridLength) . "\" align=\"center\" valign=\"top\" width=\"60\" class=\"timeborder\">$key</td>\n";
				$daydisplay .= '<td bgcolor="#a1a5a9" width="1" height="' . $this->gridLength . '"></td>'."\n";
			} else {
				$daydisplay .= '<tr>'."\n";
				$daydisplay .= '<td bgcolor="#a1a5a9" width="1" height="' . $this->gridLength . '"></td>'."\n";
				...
				for ($i=0;$i<sizeof($event_length);$i++) {
					...
					switch ($event_length[$i]['state']) {
						case 'begin':
						  ...
						  $event_start 	= date ($this->timeFormat, $event_start);
						  $event_end	= date ($this->timeFormat, $event_end);
 						  $event_calno  = $this_time_arr[$uid]['calnumber'];
 						  $event_recur  = $this_time_arr[$uid]['recur'];
 						  $event_status = strtolower($this_time_arr[$uid]['status']);
						  $event_calno  = (($event_calno - 1) % $this->unique_colors) + 1;
						  ...
						  // Start drawing the event
						  $event_temp  = $loop_event;
						  $event 	   = openevent($this->getdate, $cal_time, $uid, $this_time_arr[$uid], 0, 0, 'ps');
						  $event_temp  = str_replace('{EVENT}', $event, $event_temp);
						  $event_temp  = str_replace('{EVENT_START}', $event_start, $event_temp);
	...
	function tomorrows_events() {
		...
		if (is_array($this->master_array[$this->next_day]) && sizeof($this->master_array[$this->next_day]) > 0) {
			foreach ($this->master_array[$this->next_day] as $cal_time => $event_times) {
				foreach ($event_times as $uid => $val) {
					$event_text = stripslashes(urldecode($val["event_text"]));
					$event_text = strip_tags($event_text, '<b><i><u>');
					if ($event_text != "") {
						if (!isset($val["event_start"])) {
							$return_adtmp = openevent($this->next_day, $cal_time, $uid, $val, $this->tomorrows_events_lines, 21, 'psf');
							$replace_ad  .= str_replace('{T_ALLDAY}', $return_adtmp, $loop_t_ad);
						} else {
							$return_etmp  = openevent($this->next_day, $cal_time, $uid, $val, $this->tomorrows_events_lines, 21, 'ps3');
	...
	function draw_month($template_p, $offset = '+0', $type) {
		...
		if ($type != 'medium') {
			$fake_getdate_time 	= strtotime($this->this_year.'-'.$this->this_month.'-15');
			$fake_getdate_time	= strtotime("$offset month", $fake_getdate_time);
		} else {
			$fake_getdate_time 	= strtotime($this->this_year.'-'.$offset.'-15');
		}
		...
		if ($type == 'small') {
			$langtype = $this->daysofweekreallyshort_lang;
		} elseif ($type == 'medium') {
			$langtype = $this->daysofweekshort_lang;
		} elseif ($type == 'large') {
			$langtype = $this->daysofweek_lang;	
		}
		...
		$start_day 			= strtotime(dateOfWeek($first_of_month, $this->week_start_day));
		$i 					= 0;
		$whole_month 		= TRUE;
		...
		do {
			...
			#$temp_middle			= $loop_md;
			$switch					= array('ALLDAY' => '', 'CAL' => $this->cal, 'MINICAL_VIEW' => $this->minical_view);
			...
			if (isset($this->master_array[$daylink])) {
				if ($type != 'small') {
					foreach ($this->master_array[$daylink] as $cal_time => $event_times) {
						foreach ($event_times as $uid => $val) {
							$event_calno 	= $val['calnumber'];
							$event_calno	= (($event_calno - 1) % $this->unique_colors) + 1;
							if (!isset($val['event_start'])) {
								if ($type == 'large') {
									$switch['ALLDAY'] .= '<div class="V10"><img src="templates/'.$this->template.'/images/monthdot_'.$event_calno.'.gif" alt="" width="9" height="9" border="0" />';
 									$switch['ALLDAY'] .= openevent($daylink, $cal_time, $uid, $val, $this->month_event_lines, 15, 'psf');
 									$switch['ALLDAY'] .= (isset($val['location']) && $val['location'] != '') ? $val['location']."<br />" : '';
									$switch['ALLDAY'] .= '</div>';
								} else {
									$switch['ALLDAY'] .= '<img src="templates/'.$this->template.'/images/allday_dot.gif" alt=" " width="11" height="10" border="0" />';
								}
							} else {	
								$start2		 = date($this->timeFormat_small, $val['start_unixtime']);
								if ($type == 'large') {
									$switch['EVENT'] .= '<div class="V9"><img src="templates/'.$this->template.'/images/monthdot_'.$event_calno.'.gif" alt="" width="9" height="9" border="0" />';
 									$switch['EVENT'] .= openevent($daylink, $cal_time, $uid, $val, $this->month_event_lines, 10, 'ps3', "$start2 ").'';
 									$switch['EVENT'] .= (isset($val['location']) && $val['location'] != '') ? "<br />".$val['location']."<br />" : '';
									$switch['EVENT'] .= '</div>';
								} else {
									$switch['EVENT'] = '<img src="templates/'.$this->template.'/images/event_dot.gif" alt=" " width="11" height="10" border="0" />';
								}
							}
						}
					}
		...
		$return = str_replace('{CAL}', $this->cal, $return);
	...
	function monthbottom() {
		...
		$m_start = $this->this_year.$this->this_month.'01';
			...
			if (isset($this->master_array[$m_start])) {
				...
				#	$switch['START_DATE'] 	= localizeDate ($this->dateFormat_week_list, $u_start);
					...
					foreach ($event_times as $uid => $val) {
						...
						if (!isset($val['event_start'])) {
							$switch['START_TIME'] 	= $this->lang['l_all_day'];
							$switch['EVENT_TEXT'] 	= openevent($m_start, $cal_time, $uid, $val, $this->month_event_lines, 15, 'psf');
							...
							$event_start = date($this->timeFormat, $val['start_unixtime']);
							$event_end   = date($this->timeFormat, @strtotime ($event_end));
		...
		#	unset ($switch);
		} while ($this->this_month == $check_month);
	...
	function Page($file = 'std.tpl') {
		...
		if (!file_exists($file)){
			...
			$file = str_replace("templates/$this->template","templates/default",$file); 
	...
	function parse($file) {
		...
		if (basename(dirname($file)) == "$this->template" || $file =='./functions/event.js'){
			if (!is_file($file)){
				#look for it in default if not found
				$file = str_replace("templates/$this->template","templates/default",$file); 
			}
	...
	function output() {
		...
		if (sizeof($match) > 0) {
			...
			foreach ($match[1] as $key => $val) {
				if ($match[1][$i] == 'SMALL') {
					$template_file 	= $this->parse(BASE.'templates/'.$this->template.'/month_small.tpl');
					$type 			= 'small';
					$offset 		= $match[2][$i].$match[3][$i];
				} elseif ($match[1][$i] == 'MEDIUM') {
					$template_file 	= $this->parse(BASE.'templates/'.$this->template.'/month_medium.tpl');
					$type 			= 'medium';
					$offset 		= $match[3][$i];
				} else {
					$template_file 	= $this->parse(BASE.'templates/'.$this->template.'/month_large.tpl');
					$type 			= 'large';
					$offset 		= $match[2][$i].$match[3][$i];
				}
		...
		$generated1 = number_format(($php_ended-$this->php_started),3);
		$generated2 = number_format(($php_ended-$this->template_started),3);
		...
		if ($this->cpath){
			$this->page = str_replace('&amp;getdate', "&amp;cpath=$this->cpath&amp;getdate", $this->page);
		}
		print($this->page);
	}
}
